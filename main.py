...
URL = 'https://www.metacritic.com/browse/games/release-date/available/'
HEADERS = {
...
CSV = 'data.csv'
PLATFORMS = ['ps4', 'ps5', 'xboxone', 'switch', 'pc', 'ios']  # just type in here platforms to scrape
...
def parser(p):
    ...
    soup_start = request_and_soup(URL + p + '/', params='')
    pagination = get_num_pages(soup_start)
    data_file = create_file(p + '.csv')
    for page in range(0, pagination):
        print(f'\nParsing page {page}/{pagination}')
        current_soup = request_and_soup(URL + p, params={'page': page})
        ...
        for link in current_links:
            try:
                ...
                print(error)
                continue
        sleep(3)  # to lower consumption of metacritic's resources
